...
import { ActionIcon, Pagination, Select, Button, Container, Group, Modal, Paper, Stack, Switch, Table, Text, TextInput, Title, rem } from '@mantine/core';
...
export default function AdminCategories() {
  ...
  // filtros/paginação simples (pode estender depois)
...
const totalPages = Math.max(1, Math.ceil(total / pageSize));
  ...
  const mCreate = useMutation({
    ...
    setPage((p) => p - 1); // volta pra página anterior e a query roda de novo
   } else {
    ...
    setConfirmDelete(null);
      ...
      onError: (err: any) => notifyError(String(err?.response?.data ?? 'Erro ao criar categoria')),
  });
      ...
      <main className={classes.main}>
        <div className={classes.shell}>
          ...
          <div className={classes.topbar}>
            <HeaderAdmin title="Cadastro de categorias" onLogout={handleLogout}/>
          </div>
      ...
      >
        <Stack>
          ...
          <Group justify="flex-end">
            ...
            <Button color="red" onClick={() => confirmDelete && mDelete.mutate(confirmDelete.id)} loading={mDelete.isPending}>
              Excluir
            </Button>
          </Group>
        </Stack>
